...
/**
 * @ngdoc function
 * @name chitchat.controller:GroupsendCtrl
 * @description
 * # MainCtrl
 * Controller of chitchat
 */
angular.module('chitchat')
	.controller('GroupsendCtrl', function($scope, $rootScope, $state, $uibModal, $filter, $localStorage, QuickbloxService, Config, $http, $httpParamSerializer) {
		$scope.init = function() {
			$scope.per_page = 250;
		...
		$scope.filterUsers = function(event, value) {
			if (event.keyCode === 13) {
				...
				var is_date = date instanceof Date && !isNaN(date.valueOf());
				console.log('mass search:' + is_date + ", value:" + value);
				if (is_date) {
					var page_num = 2000;
					$scope.qbParams = {page: 1, per_page: page_num, user_created: value};
					$scope.pagination.page = 1;
					$scope.pagination.per_page = page_num;
				} else {
					$scope.qbParams = {page: 1, per_page: $scope.per_page};
					$scope.pagination.page = 1;
					$scope.pagination.per_page = $scope.per_page;
				}
				$scope.filterText = value;
				$scope.getUsers();
			}
		...
		$scope.getUsers = function() {
			var url = Config.api.url + "?action=getMassUser";
			var qs = $httpParamSerializer($scope.qbParams);
			url = url + "&" + qs;
			$http.get(url).then(function(res) {
				//console.log(res);
				$scope.massUsers = res.data.values;
				if ($scope.gridOptions.api !== null) {
					$scope.gridOptions.api.setRowData($scope.massUsers);
				}
		...
		$scope._recurSendMultiChat = function(message) {
			if ($scope.count < $scope.total) {
				var delay_count = 100;
				if ($scope.count % delay_count) {
					$scope._sendMultiChat($scope.count, message);
				} else {
					console.log("delay 10s and user count:" + $scope.count);
					setTimeout(function() {
						$scope._sendMultiChat($scope.count, message);
					}, 10000);
				}
